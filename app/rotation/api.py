...
class PrefroshComments(Resource):
    ...
    def get(self, prefrosh_id):
        ...
        if is_admin():
            comments = Feedback.query.filter_by(Feedback.prefrosh_id=prefrosh_id).all()
        else:
            comments = Feedback.query.filter(or_(Feedback.user_id==user.id, Feedback.user_id==0)).filter_by(Feedback.prefrosh_id=prefrosh_id).all()
        return {
